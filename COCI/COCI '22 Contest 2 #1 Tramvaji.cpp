...
int n, start, ender;
ll dp[1002], ans = 99999999999999, dur, stat;
string s;
void solve(){
	cin >> n;
	...
	for (int i = 2; i<=n; ++i){
		...
		if (s == "Patrik"){
			...
			v.push_back(make_pair(dp[stat] + dur, i));
			dp[i] = v.back().first;
		}
	}
	sort(v.begin(), v.end());
	for (int i = 1; i<v.size(); ++i){
		...
		if (v[i].first - v[i - 1].first < ans){
			ans = v[i].first - v[i - 1].first;
			start = v[i - 1].second, ender = v[i].second;
		}
	}
	cout << ans << ' ' << start << ' ' << ender << '\n';
}
